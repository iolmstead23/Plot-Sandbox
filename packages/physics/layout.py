"""Initial layout: radial shell placement and BFS-landmark placement.

Two strategies are available:

  weight_radial (original)
    Nodes placed on radial shells based on weight. Heavy nodes near focus, light
    nodes on the outer shell. Topology-blind — the force simulation must discover
    graph structure from scratch.

  bfs_landmark (default)
    Nodes placed using BFS distances from k landmark vertices. Nodes that are
    graph-theoretically close start spatially close, reducing ticks to convergence.

layout_noise (weight_radial only) controls shell randomization:
  0.0 — deterministic sorted shells; r(weight, distance) = -1.0.
  1.0 — radii shuffled randomly across nodes; r0 ≈ 0.
  (0,1) — linear blend of sorted and shuffled.
"""
